---
description: "Accessibility review rubric and dual-format ADO + GitHub backlog handoff with inlined planning disclaimer"
...
* The planning disclaimer block is emitted verbatim in every output file.
* `state.disclaimerShownAt` is set.
* `state.noticeLog` contains a handoff disclaimer entry for each output artifact that emitted the disclaimer block.
  ...
  "noticeLog": [
    {
      ...
      "source": ".github/instructions/accessibility/accessibility-backlog-handoff.instructions.md",
      "details": {
        "phase": "backlog-handoff",
...
* Editing `shared/disclaimer-language.instructions.md` to add an accessibility variant. The L7 lever pins the disclaimer text to this file.
